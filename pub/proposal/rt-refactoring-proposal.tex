...
%Since both are open source, we can and will optimize the
...
clone annotations covering the entire file that developers are working in 
could be highly distracting.  As such, it makes sense to reduce the visual clutter
by only showing the clones that overlap the area in which the developer is currently typing.
...
\begin{itemize}
  ...
  \item $\textsc{Raw}_{\text{clone},\text{action}}$ is an action-specific score for the clone
    ...
    number of non-whitespace, non-Javadoc characters.
