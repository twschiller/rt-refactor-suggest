...
\begin{align*}
  \textsc{Adj} = & \left[ \textsc{RAW}_{\text{clone},\text{action}} * \left(\frac{\textsc{Pref}_{\text{action}}}{\sum{\textsc{Pref}}}\right) \right] \\
...
\textsc{Raw}_{\text{action,clone}}\textsc{Pref}_{\text{action}} > \textsc{Raw}_{\text{action,clone}}$.
...
variables declared prior to the statmements --- excluding static
fields:
...
\begin{itemize}
  ...
  \item the percentage of the new code in the final method that is a
    ...
    judgement with the aide of a clone detector.
\end{itemize}
