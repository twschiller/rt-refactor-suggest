...
\paragraph{Refactoring clones}

There has been significant research in how to refactor code containing clones
to improve code maintenance and readability.  Fanta and Rajlich propose a number of
...
present a C++ case study which demonstrates that code refactoring is an important 
addition to clone detection.

Higo et al present Aries, a tool which integrates various clone information to present 
to the user \cite{Higo2008}.  It shows the cloned blocks of code and presents refactoring options such as method extraction , but it also displays various metrics, such as position in the class hierarchy, and number of external variables. These metrics are intended to help the programmer decide which refactoring, if any, is  appropriate. 
