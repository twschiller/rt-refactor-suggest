...
We have evaluated the tool via controlled user study. We developed a
fairly small Java codebase (13 files, ~900 total lines of code) for this
purpose.  The code is an image library capable of dealing with custom-format high-fidelity
images with over 256 colors per channel.  Several complex image tranformations
are implemented, which perform tasks such as rudimentary edge detection and 
smoothing. A GUI is provided to demonstrate some of the transformations.
...
We also included a maintenance task, which is introduced once the development task is complete.    The task consists of fixing a bug in which certain functions do not set the alpha channel properly.  We provided each user with a bug report and the location of the buggy method in the source code. We indicate that the same bug might occur elsewhere in the codebase, but do not elaborate.  In reality, there are two locations where the same bug occurs.  One is a direct, copy-and paste clone of the original section, while the other only clones part of the section and uses it to complete a different task (making an image more blocky instead of shrinking it). Each location has a comment that mentions there might be a bug in how it handles alpha.  These two locations have no public unit test that exposes the bug.

%\todo{Characterize the tasks and code base} 

The subjects were two grad students who had experience using the
Java development environment. Each used the Eclipse development
environment with the tool and its logging code.
