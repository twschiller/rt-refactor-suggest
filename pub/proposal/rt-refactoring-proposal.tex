...
(the clone length considered was 50 characters).
~\cite{Yamashina2008}. 
...
Unifying code written by multiple developers may improve
code consistency, readability, and modularity;
We identify three actions that
reduce development and mainenance cost due to rewriting and
...
during maintenance.  As such, may not be possible to parse the source
file, build an AST, or type the AST. Given this, text-based detectors
are advantageous because they can be run during active
development. When a program is parsable or compilable,
more-sophisticated dectors that use ASTs or program
...
\todo{Motivate why an adaptive approach is necessary: some clone
  suggestions aren't good, the display to the user is intentionally
  intrusive so false positives are costly}
...
\begin{itemize}
  ...
  \item $I_{clone}$ is the number of times one or more development
    actions occured between clone views (or since the clone was last
    viewed)
...
either (1) switched to another task, or (2) have explicity determined
...
they are post-hoc, so in constrast to our technique
...
is appropriate. They do not, however, consider how to automcatically 
propose the most helpful refactoring(s) based on these metrics, in constract 
