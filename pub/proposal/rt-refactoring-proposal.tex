...
actions to the participants, aiding development.
...
frustrating the participant. The participant spent significant time and effort
adapting the filtering code to meet the smoothing specification, 
during which time the tool continued to annotate the code
as a clone. Upon implementing the first stage of the image
...
strategy. Furthermore, false postives in clone detection can be harmful, 
as the developer may waste time discovering that code which is structurally similar is, in fact, 
behaviorally different than what is required. Future work ought to explore (1) how
...
During the maintenance task, we expected participants to start looking at the code in 
...
He indicated that he found the tool useful for finding clones during maintenance.
